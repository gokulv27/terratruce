    ...
    <div className="col-span-1 bg-surface/80 backdrop-blur-xl border border-brand-primary/20 rounded-3xl p-6 shadow-2xl relative overflow-hidden h-full flex flex-col">
        <div className="flex items-center justify-between mb-6 relative z-10">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h3 className="font-black text-lg text-text-primary tracking-tight leading-none">Trending Markets</h3>
                    <p className="text-[10px] font-bold text-text-secondary uppercase tracking-wider mt-1">{region || 'Global'} Top 5</p>
                </div>
            </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-1 relative z-10 custom-scrollbar">
            ...
            {items.map((item, idx) => (
                ...
                >
                    <div className="col-span-1 font-black text-xs text-text-secondary/50 group-hover:text-brand-primary">{idx + 1}</div>
                    <div className="col-span-8 font-bold text-sm text-text-primary truncate pr-2">{item.name}</div>
...
const MarketStatCard = () => (
    <div className="bg-gradient-to-br from-emerald-400 via-teal-500 to-cyan-600 dark:from-brand-primary dark:to-brand-secondary rounded-3xl p-6 shadow-xl shadow-emerald-500/30 dark:shadow-brand-primary/20 text-white relative overflow-hidden group hover:scale-[1.02] transition-transform">
        ...
        <div className="relative z-10">
            <div className="text-xs font-bold text-white/90 uppercase tracking-wider mb-2">Highest Yield</div>
    ...
    <div className="bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-800 dark:to-slate-900 backdrop-blur-xl border-2 border-blue-200 dark:border-white/10 rounded-3xl p-1 shadow-xl shadow-blue-500/20 dark:shadow-sky-500/10 relative overflow-hidden group hover:border-indigo-300 dark:hover:border-brand-primary/30 transition-all">
        <div className="h-full w-full bg-gradient-to-br from-blue-100/50 to-indigo-100/50 dark:from-brand-secondary/5 dark:to-transparent rounded-[20px] flex items-center justify-center relative overflow-hidden">
            ...
            <div className="absolute inset-0 grid grid-cols-6 grid-rows-6 gap-2 p-4 opacity-30 dark:opacity-20">
                {[...Array(36)].map((_, i) => (
                    <div key={i} className="bg-gradient-to-br from-blue-500 to-indigo-600 dark:from-brand-primary dark:to-brand-primary rounded-full w-1 h-1" />
                ))}
            </div>
            {/* Pulse */}
            <div className="relative z-10">
            ...
            <div className="absolute bottom-3 left-0 w-full text-center">
                <span className="text-[10px] font-bold bg-gradient-to-r from-blue-600 to-indigo-700 dark:from-brand-primary dark:to-brand-primary bg-clip-text text-transparent bg-white/90 dark:bg-white/80 px-2 py-1 rounded-full backdrop-blur shadow-md">Live Risk Map</span>
            </div>
        </div>
    </div>
);
        ...
        <div ref={containerRef} className="h-full overflow-y-auto custom-scrollbar bg-background text-text-primary overflow-x-hidden font-sans">
                ...
                <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10 w-full">
                    ...
                    <div className="space-y-8 pt-4 text-left">
                        ...
                        <div className="hero-content flex flex-wrap gap-4">
                            <Link id="search" to="/analyze" className="px-8 py-4 bg-brand-primary text-white rounded-2xl font-bold hover:bg-brand-primary/90 hover:scale-105 transition-all shadow-xl shadow-brand-primary/20 flex items-center gap-2">
                                Start Analysis <ArrowRight className="h-5 w-5" />
                            </Link>
                            <Link id="calculator" to="/market" className="px-8 py-4 bg-white border border-border text-brand-primary rounded-2xl font-bold hover:border-brand-primary hover:bg-brand-primary/5 transition-all flex items-center gap-2">
            ...
            <div className="py-24 px-6 container mx-auto">
                <div className="text-center mb-16 max-w-2xl mx-auto">
                    <h2 className="text-3xl font-bold mb-4 text-brand-primary">Designed for the Modern Investor</h2>
                    <p className="text-text-secondary">Beautiful tools for serious financial decisions.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <FeatureCard icon={Search} title="Smart Search" desc="Type less, find more with predictive location search." />
                    <FeatureCard id="analyze" icon={Shield} title="Risk Radar" desc="Visualize flood zones and crime heatmaps instantly." />
                    <FeatureCard icon={Calculator} title="ROI Projection" desc="Customizable 30-year cash flow forecasts." />
                </div>
            </div>
        </div>
    );
