...
const RiskMetricCard = ({ icon: Icon, title, score, status, factors, description }) => {
    ...
    const getColorScheme = (score) => {
        if (score >= 70) return {
            ...
            badge: 'bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-500/30'
        };
        if (score >= 40) return {
            ...
            badge: 'bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-900/40 dark:text-yellow-300 dark:border-yellow-500/30'
        };
        return {
            ...
            badge: 'bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-500/30'
        };
    };
    ...
    return (
        <div className={`rounded-xl bg-gradient-to-br ${colors.bg} border ${colors.border} overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-teal-500/10`}>
            {/* Header */}
            <div
            ...
            {isExpanded && hasFactors && (
                <div className="px-4 pb-4 space-y-2 bg-surface-elevated/50 border-t border-border dark:bg-gray-900/40 dark:border-gray-700/30">
                    <p className="text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2 mt-3">Contributing Factors</p>
                    {factors.map((factor, idx) => (
                        <div key={idx} className="flex items-start gap-2 p-2 bg-white rounded-lg border border-border dark:bg-gray-800/40 dark:border-gray-700/30">
                            <div className="mt-1">
