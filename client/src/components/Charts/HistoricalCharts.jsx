...
const HistoricalCharts = ({ data }) => {
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            return (
                <div className="bg-gray-800/95 backdrop-blur-sm border border-purple-500/30 rounded-lg p-3 shadow-xl">
    ...
    return (
        <div className="bg-gray-800/40 border border-gray-700/30 rounded-xl p-5">
                ...
                {chartTabs.map(tab => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeChart === tab.key
            ...
            {historical_trends.development_timeline && historical_trends.development_timeline.length > 0 && (
                <div className="mt-6 pt-5 border-t border-gray-700/30">
                    <h4 className="text-sm font-bold text-white mb-3">Development Timeline</h4>
                    <div className="space-y-2">
                        {historical_trends.development_timeline.map((event, idx) => (
                            <div key={idx} className="flex gap-3 items-start">
                                <div className="flex-shrink-0 w-16 text-xs font-semibold text-purple-400 mt-1">
                                    {event.year}
                                </div>
                                <div className="flex-1 bg-gray-700/30 rounded-lg p-2 border border-gray-600/30">
                                    {event.major_events && event.major_events.map((evt, eIdx) => (
                                        <p key={eIdx} className="text system-xs text-gray-300">• {evt}</p>
                                    ))}
                                </div>
                            </div>
                        ))}
