...
//     const [projectionYears, setProjectionYears] = useState(5); // Default 5 years
...
//         if (!score) return;
//         // Logic: High Risk (>70) = Low Appr (2%), High Rate (7.5%)
//         // Low Risk (<30) = High Appr (6%), Low Rate (5.5%)
//         if (score > 70) {
//             setAppreciation(2);
//             setInterestRate(7.5);
//         } else if (score < 30) {
...
//     // 1. Handle Incoming State from Router (History/Analyze)
//     useEffect(() => {
...
//     // 2. Direct Location Search Handler
//     const handleDirectSearch = async (locName) => {
//         if (!locName) return;
//         setAnalyzing(true);
//         setCurrentLocation(locName);
//         try {
//             // Fetch real risk score to power the calculator
...
//         const numberOfPayments = 30 * 12; // 30 yr fixed assumption for payment
...
//         // Custom Year Projection
//         const projection = [];
//         let currentValue = purchasePrice;
//         let cumulativeEquity = downPayment;

//         for (let i = 0; i <= projectionYears; i++) {
//             // Simple equity calculation (principal paydown + appreciation)
//             // Note: This is a simplified projection for UI
//             const principalPaydownYearly = (mortgagePayment * 12) * 0.3; // Rough approx average over 30 yrs
//             if (i > 0) cumulativeEquity += principalPaydownYearly + (currentValue * (appreciation / 100)); // Add appreciation manually to equity or just use Value - Debt?

//             // Better: Value - Remaining Debt
//             // Debt starts at loanAmount. reduced by principal portion.

//             projection.push({
//                 year: `Year ${i}`,
//                 value: Math.round(currentValue),
//                 equity: Math.round(cumulativeEquity) // Simplified
//             });
...
//                             {isAutoEstimated && (
//                                 <span className="auto-est-notice flex items-center gap-1 text-[10px] bg-brand-secondary/10 text-brand-secondary px-2 py-0.5 rounded-full font-bold border border-brand-secondary/20 animate-pulse">
...
//                         <div className="absolute -right-10 -top-10 w-32 h-32 bg-brand-primary/5 rounded-full opacity-0 pointer-events-none" />
...
//                     {/* Chart Container */}
//                     <div className="bg-surface rounded-3xl border border-border transition-all duration-300 shadow-xl p-6 calc-card flex flex-col relative z-0">
...
//                                     {projectionYears}-Year Forecast @ <span className="text-brand-primary">{appreciation}%</span> Appreciation
//                                 </p>
//                             </div>
...
//                                     />
//                                     <Tooltip
//                                         contentStyle={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)', borderRadius: '16px', boxShadow: '0 10px 30px -5px rgba(0,0,0,0.1)' }}
//                                         itemStyle={{ color: 'var(--brand-primary)', fontWeight: 'bold' }}
//                                         formatter={(value) => [`${currency}${value.toLocaleString()}`, 'Projected Value']}
//                                     />
//                                     <Area
//                                         type="linear"
//                                         dataKey="value"
...
//                                         fillOpacity={0.1}
//                                         animationDuration={0}
//                                         dot={{ stroke: '#8B5CF6', strokeWidth: 2, fill: 'var(--surface)', r: 4 }}
...
//                 {isPositive && <div className="p-0.5 rounded-full bg-green-100 text-green-600"><TrendingUp className="h-3 w-3" /></div>}
